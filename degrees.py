...
def shortest_path(source, target):
    ...
    """
    # Keep track number of explored nodes
    num_explored = 0
    ...
    while True:
        ...
        # If there is no node in the frontier, raise Error
        if frontier.empty():
        ...
        for movie, actor in neighbors_for_person(node.state):
            if actor not in explored and not frontier.contains_state(actor):
                ...
                # Check if child node is target
                if child.state == target:

                    path = []

                    # Add all walked connections to path
                    while child.parent is not None:
                        path.append((child.action, child.state))
                        child = child.parent
                    path.reverse()
                    return path
                # Add child to frontier if it is not the target node
                frontier.add(child)
